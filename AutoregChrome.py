...
if chrome_path:
    ...
    print(f"Используется Chrome по пути: {chrome_path}")
else:
    print("Chrome не найден в стандартных расположениях")
    chrome_path = input("Введите полный путь к chrome.exe: ")
    if os.path.exists(chrome_path):
        chrome_options.binary_location = chrome_path
    else:
        raise FileNotFoundError("Указанный путь к Chrome не существует")
